...
function prepareApiData(formData) {
    const toBoolean = (value) => {
        ...
        if (typeof value === 'string') {
            ...
            return normalized === 'yes' || normalized === 'yes - original' || normalized === 'yes - written' || normalized.startsWith('yes');
        }
        return false;
    };
    ...
    const apiPayload = {
        ...
        cheque_present: cheque.cheque_number ? true : dishonour.bank_memo_received,
        dishonour_memo: evidence.dishonour_memo_available,
        notice_sent: notice.notice_sent,
        debt_proven: transaction.debt_acknowledged || toBoolean(formData.transaction?.supporting_documents),
        description: [
            transaction.purpose,
...
function validateCaseCompleteness() {
    ...
    const hasEvidence = formData.evidence && Object.keys(formData.evidence).some(key => formData.evidence[key]);
    ...
    const noticeSent = formData.notice?.notice_sent === 'Yes' || formData.notice?.notice_sent === true;
    const hasDocuments = formData.evidence?.bank_memo || formData.evidence?.transaction_proof;
...
function renderScoreExplanation(data) {
    ...
    if (score < 50) {
        ...
        if (!formDataStr.includes('notice_sent') || formDataStr.includes('"notice_sent":""')) {
            missingElements.push('Legal notice not sent (mandatory requirement)');
        }

        if (!formDataStr.includes('bank_memo') && !formDataStr.includes('transaction_proof')) {
            missingElements.push('No supporting documents provided');
        }

        if (!formDataStr.includes('transaction_date') || formDataStr.includes('"transaction_date":""')) {
...
function renderImprovementSuggestions(data) {
    ...
    if (!formDataStr.includes('notice_sent') || formDataStr.includes('"notice_sent":"No"')) {
        suggestions.push({
            icon: 'fa-file-alt',
    ...
    if (!formDataStr.includes('bank_memo') || formDataStr.includes('"bank_memo":""')) {
        suggestions.push({
            icon: 'fa-university',
    ...
    if (!formDataStr.includes('transaction_proof') || formDataStr.includes('"transaction_proof":""')) {
        suggestions.push({
            icon: 'fa-receipt',
    ...
    if (!formDataStr.includes('witness_')) {
        suggestions.push({
            icon: 'fa-users',
...
function buildDraftPrefill() {
    const map = {};
    try {
        const s = { ...(formData.case_identity || {}), ...(formData.parties || {}), ...(formData.transaction || {}), ...(formData.cheque || {}), ...(formData.dishonour || {}), ...(formData.notice || {}) };
